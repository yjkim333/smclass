...
# # 1줄 for문(for문을 1줄로 쓰기) -> 리스트내포(컴프리헨션)
...
# key 만 가져올때 - nameDic.keys()
# value - nameDic.valuse()
...
# nameDics = sorted(nameDic.items(),key=operator.itemgetter(1))  # [1]번째로 순차정렬 key=operator.itemgetter(1) -> import operator 필요
# nameDics = sorted(nameDic.items(),key=operator.itemgetter(1),reverse=True)  # [1]번째로 역차정렬
