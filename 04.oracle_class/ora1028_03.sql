...
-- number(3)
-- varchar2(20) - 문자열
-- no,name,kor,eng,math,total,avg,rank 컬럼을 구성해서 테이블 구현
...
total number(3),
avg number(6,2),
rank number(4)  -- 0~9999등까지 
);

-- 홍길동,유관순,이순신... 데이터 입력
insert into students values(
1,'홍길동',100,100,99,299,299/3,0
);
insert into students values(
...
insert into students values(
4,'강감찬',60,65,67,192,192/3,0
);
insert into students values(
5,'김구',100,100,84,284,284/3,0
);
insert into students values(
6,'김유신',80,80,71,231,231/3,0
);
insert into students values(
7,'홍길자',100,100,90,290,290/3,0
);
...
-- scale -> 소수점 이하 자리수 / 생략하면 소수점이하는 반올림되어 정수 값으로 표현


create table no_tab (
no number,
name varchar2(20),
avg1 number,    -- 소수점 지정 없이 선언
avg2 number(5,2)  -- 소수점 지정 -> 123.45  총 5자리, 소수점 2자리
);
...
-- varchar2
-- 가변길이 문자열 저장
-- 입력한 데이터 만큼만 저장공간을 할애 -> 메모리 낭비를 줄일 수 있다.

-- char
-- 고정길이 문자 데이터 저장
-- 입력된 자료의 길이와 상관없이 정해진 길이 만큼 저장 영역 차지
-- 편차가 심한 데이터를 입력할 경우 저장 공간이 낭비 됨
-- varchar2 보다 속도가 빠름


-- select - 검색
-- select 컬럼명 from 테이블명
-- select * from 테이블명  * => 모든 컬럼을 의미
select employee_id,emp_name,salary from employees;
select EMPLOYEE_ID from EMPLOYEES;  -- 대소문자 구분하지 않음

select emp_name,SALARY from employees where emp_name = 'Pat Fay'  -- 저장된 데이터는 대소문자 구분

select * from employees;

-- 사원번호ㅡ사원명,전화번호, 입사일
select employee_id,emp_name,phone_number,hire_date from employees;
-- 사원번호 사원명 직급 중 SH_CLERK 검색
...
-- 데이터 추가(insert), 검색(select), 삭제(delete), 수정(update)


-- 삭제
-- where 절 - 컬럼에 해당되는 값
-- delete member; -- 모든 행 삭제
...
-- 취미가 swim 인 사람 -> bike 로 변경
update member set hobby = 'bike' where hobby = 'swim';
rollback;


-- oracle sql 종류 - DDL, DML, DCL
-- DML(데이터 조작어)에만 commit rollback  있다.
...
select * from employees;
-- 사원 월급(달러)을 원화로 환산 하시오.
select emp_name,salary,to_char(salary*1384.66,'999,999,999.99') from employees;
